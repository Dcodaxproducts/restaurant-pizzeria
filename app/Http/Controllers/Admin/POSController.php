...
class POSController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $categories = $this->category->active()->where('position', 0)->get();
        ...
        $customers = $this->user->select('id', 'f_name', 'l_name')->get();
        $order = null;
        $addOns = collect();
        $businessSettings = [];
        ...
        if (session('last_order')) {
            $order = $this->order->with(['details.product', 'customer', 'delivery_address'])
                ->where('id', session('last_order'))
                ->first();
            $addOns = $this->getOrderAddOns($order);
            $businessSettings = $this->businessSettings(['phone', 'overall_tax']);
        }

        $selectedCategory = $category;
        $cartViewData = $this->getCartViewData();
        return view('admin-views.pos.index', array_merge(
            compact('categories', 'products', 'category', 'keyword', 'current_branch', 'branches', 'selected_customer', 'selected_table', 'tables', 'selectedCategory', 'restaurantLogo', 'mapApiClientKey', 'customers', 'order', 'addOns', 'businessSettings'),
            $cartViewData
        ));
    }
    ...
    public function getProducts(Request $request)
    {
        ...
        return view('admin-views.pos.order.partials._product_partial', compact('categories', 'products', 'category', 'current_branch', 'branches', 'selected_customer', 'selected_table', 'tables', 'selectedCategory'))->render();
    }
    ...
    public function quick_view(Request $request): JsonResponse
    {
        ...
        return response()->json([
            'success' => 1,
            'view' => view('admin-views.pos._quick-view-data', compact('product', 'selectedAddOns'))->render(),
        ]);
    }
    ...
    {
        return view('admin-views.pos._cart', $this->getCartViewData());
    }
    ...
    public function order_list(Request $request): Renderable
    {
        ...
        $orders = $query->paginate(Helpers::getPagination())->appends($query_param);
    ...
    public function order_details($id): Renderable|RedirectResponse
    {
        ...
        return view('admin-views.order.order-view', compact('order', 'delivery_man', 'timeZone'));
    }
    ...
    public function generate_invoice($id): JsonResponse
    {
        ...
        return response()->json([
            'success' => 1,
            'view' => view('admin-views.pos.order.invoice', compact('order', 'addOns', 'businessSettings'))->render(),
        ]);
    }
    ...
    private function getCartViewData(): array
    {
        ...
        return [
            'cartProducts' => $this->product->whereIn('id', $productIds)->get()->keyBy('id'),
            'cartAddOns' => AddOn::whereIn('id', $addOnIds)->get()->keyBy('id'),
            'overall_tax' => (double) ($this->businessSettings(['overall_tax'])['overall_tax'] ?? 0),
        ];
    }
