...
if (in_array($request->payment_method, ['stripe', 'online_payment'])) {
            $stripe_payment = $this->verifyStripePaymentIntent($request);

            if (!$stripe_payment['status']) {
                return response()->json([
                    'errors' => [
                        ['code' => 'payment_method', 'message' => $stripe_payment['message']]
                    ]
                ], 403);
            }
        }
		
		 if ($request->payment_method == 'bancontact' && isset($request->transaction_reference)) {
            // Try to find the order with the given payment method and transaction reference
            $existingOrder = Order::where([
                'payment_method' => 'bancontact',
                'transaction_reference' => $request->transaction_reference
            ])->first();

            // If the order exists, redirect to the failure page
            if ($existingOrder) {
              return response()->json([
		...
		if (in_array($request->payment_method, ['paypal', 'bancontact','paystack'])) {
			// Code for paypal or bancontact
			$payment_status = 'unpaid';
			$order_status = 'failed'; 
		
		} else if(in_array($request->payment_method, ['stripe', 'online_payment'])) {
			$payment_status = 'paid';
			$order_status = 'confirmed';
		} else if($request->payment_method == 'wallet') {
			$payment_status = 'paid';
			$order_status = 'confiremd'; 
		}else{
			$payment_status = 'unpaid';  
        ...
        try {
            $order_id = 100000 + $this->order->all()->count() + 2;
            $or = [
                'id' => $order_id,
                ...
                'delivery_charge' => $request['delivery_charge'], //$request['order_type'] != 'take_away' ? 		Helpers::get_delivery_charge($request['distance']) : 0,
                'preparation_time' => 0,
				 'wolt_promise_id' => $request['wolt_promise_id'],
                'created_at' => now(),
                'updated_at' => now()
            ];
            $total_tax_amount = 0;
			...
			if($request->payment_method !=  'bancontact'){
				try {
    ...
    private function verifyStripePaymentIntent(Request $request): array
    {
        ...
        if ($this->order
            ->whereIn('payment_method', ['stripe', 'online_payment'])
            ->where('transaction_reference', $request->transaction_reference)
            ->exists()
        ) {
            return [
                'status' => false,
                'message' => 'Stripe payment intent already used'
            ];
        }
        ...
        try {
            ...
            $expected_amount = $this->stripeAmount((float)$request['order_amount'], $currency_code);

            Stripe::setApiKey($config['api_key']);
            $payment_intent = PaymentIntent::retrieve($request->transaction_reference);
            ...
            if ((int)$payment_intent->amount !== $expected_amount) {
                return [
                    'status' => false,
                    'message' => 'Stripe payment amount does not match order amount'
                ];
            }

            if (strtoupper($payment_intent->currency) !== $currency_code) {
                return [
                    'status' => false,
                    'message' => 'Stripe payment currency does not match'
                ];
            }

            if ((string)($payment_intent->metadata->customer_id ?? '') !== (string)$request->user()->id) {
                return [
                    'status' => false,
                    'message' => 'Stripe payment customer does not match'
                ];
            }
