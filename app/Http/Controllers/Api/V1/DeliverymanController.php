...
class DeliverymanController extends Controller
{
    ...
    public function update_order_status(Request $request): JsonResponse
    {
        ...
        if($request->hasfile('image')){
             $order_image = Helpers::upload('order/', 'png', $request->file('image')) ?? 'abc';
             $order->update(['order_confirmation_image' => $order_image]);
        }
    ...
    public function get_all_orders(Request $request): JsonResponse
    {
        ...
        $orders = $this->order
            ->with(['delivery_address', 'customer', 'details.product'])
            ->withCount('details as item_count')
            ->where(['delivery_man_id' => $dm['id']])
    ...
    public function get_last_location(Request $request): JsonResponse
    {
        ...
        $last_data = $this->delivery_man
            ->where(['id' => $request['driver_id']])
            ->select('last_lat','last_long')
            ->first();

        $order = $this->order->with(['delivery_address'])->where('id', $request['order_id'])->first();
        $deliveryAddress = $order->delivery_address;
 
        $eta = Helpers::mighty_get_distance_matrix($last_data->last_lat, $last_data->last_long, $deliveryAddress['latitude'], $deliveryAddress['longitude']);
    
        if ($eta['status'] == 'OK') {
            $durationTime = $eta['rows'][0]['elements'][0]['duration']['text'];
        } else {
            $durationTime = 'Error in Google Maps API response';
        }
    

        $last_data = [
            'last_lat' => $last_data->last_lat,
            'last_long' => $last_data->last_long,
            'eta' => $durationTime
        ];
