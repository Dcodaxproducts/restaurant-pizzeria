<div style="width:370px;margin-left:18px" id="printableAreaContent">
    ...
    <div class="row">
        <div class="col-md-12">
            <fieldset style="border:2px dashed #000000; border-radius:10px;">
                <legend class="text-center">
                    Receipt
                </legend>
                <h6 class="text-order_id text-center">{{ translate('Order ID') }} : {{ $order['id'] }}</h6>
            </fieldset>
        </div>
...
<div class="row justify-content-md-end">
    <div class="col-md-9 col-lg-9">
        <dl class="row text-right" style="color: black!important;">
            ...
            @php($overall_tax = (float) ($businessSettings['overall_tax'] ?? 0))
            <dd class="col-4">{{ \App\CentralLogics\Helpers::set_symbol($overall_tax) }}</dd>
            ...
            @php($subtotal = $add_ons_cost + $item_price + $total_tax + $add_ons_tax_cost + $overall_tax)
            ...
            <dd class="col-6" style="font-size: 20px">
                {{ \App\CentralLogics\Helpers::set_symbol($subtotal - $order['coupon_discount_amount'] - $order['extra_discount']) }}
            </dd>
        </dl>
    </div>
</div>
